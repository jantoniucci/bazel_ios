...
readonly bazel_cmd=(
  ...
  "/Users/jrai/Library/Caches/bazelisk/downloads/bazelbuild/bazel-6.2.1-darwin-arm64/bin/bazel"
...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@@rules_xcodeproj~1.13.0~internal~rules_xcodeproj_generated//generator/app/xcodeproj:xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj~1.13.0~internal~rules_xcodeproj_generated/generator/app/xcodeproj/xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj~1.13.0~internal~rules_xcodeproj_generated/generator/app/xcodeproj/xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj~1.13.0~internal~rules_xcodeproj_generated/generator/app/xcodeproj/xcodeproj_target_ids")
